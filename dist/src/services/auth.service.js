...
export const authLoginServices = async (data) => {
    ...
    const checkIfPasswordMatch = await bcrypt.compare(data.password, fetchedUserData.password);
...
//Register User
export const authRegisterServices = async (data, file, token) => {
    const decryptedToken = jwt.verify(token, config.JWT_VALIDATE_SECRET);
...
//change passswod
export const changePasswordServices = async (user, data) => {
    ...
    if (!passwordCheck)
        throw new apiError(500, `failed to get user credentials`);
    const checkPassword = bcrypt.compareSync(data.old_password, passwordCheck.password);
    if (!checkPassword)
...
export const passwordResetMailServices = async (email) => {
    ...
    const token = jwt.sign({ email }, config.JWT_VALIDATE_SECRET, {
        expiresIn: config.JWT_VALIDATE_TOKEN_EXPIRES_IN || '10m'
    });
    const emailSubject = "Reset Password";
    const emailMessage = "We received a request to reset your password. Click the button below to set a new password. This link will expire in 10 minutes.";
    ...
    const info = emailDetailUtils(email, token, emailSubject, emailMessage, baseUrl, purpose);
    if (!info)
        throw new apiError(500, "unable to send otp");
    return info;
};
//send mail for registration
export const registerMailServices = async (data) => {
    ...
    const token = jwt.sign({ email, role }, config.JWT_VALIDATE_SECRET, {
        expiresIn: config.JWT_VALIDATE_TOKEN_EXPIRES_IN ||
            "10m",
    });
    const emailSubject = "User Registration";
    const emailMessage = "We received a request to create an account using this email address.To continue with registration, please verify your email by clicking the button below.";
...
//reset password
export const resetPasswordServices = async (token, data) => {
    const decryptedToken = jwt.verify(token, config.JWT_VALIDATE_SECRET);
...
//refreshAccessToken
export const refreshAccessTokenServices = async (incomingRefreshToken) => {
    if (!incomingRefreshToken)
        throw new apiError(400, "refresh Token missing");
    const token = jwt.verify(incomingRefreshToken, config.JWT_REFRESH_TOKEN_SECRET_KEY);
...
export const logOutServices = async (token) => {
    ...
    if (!blacklistToken)
        throw new apiError(404, "LogOut failed");
};
