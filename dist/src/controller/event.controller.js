...
export const getEventController = asyncHandler(async (req, res) => {
    ...
    const { event, meta } = await filterEventServices(Number(page), Number(offset), searchValue, req.user);
    return resHandler(res, 200, true, "event retrived successfully", event, meta);
});
export const searchEventController = asyncHandler(async (req, res) => {
    const page = req.query.page || 1;
    ...
    const { data, meta } = await searchEventServices(Number(page), Number(offset), search, req.user);
    return resHandler(res, 200, true, "event retrived successfully", data, meta);
});
...
export const deleteEventController = asyncHandler(async (req, res) => {
    ...
    const deleteEvent = await deleteEventServices(eventId, req.user);
...
export const getEventByStatusController = asyncHandler(async (req, res) => {
    ...
    const upperCaseStatus = Array.isArray(status)
        ...
        : status.toUpperCase();
    const { event, meta } = await getEventByStatusServices(upperCaseStatus, Number(page), Number(offset));
...
export const getAllEventController = asyncHandler(async (req, res) => {
    const page = req.query.page || 1;
    const offset = req.query.offset || 10;
    const { event, meta } = await getAllEventServices(Number(page), Number(offset));
    return resHandler(res, 200, true, "event retrived successfully", event, meta);
});
//get organized event
export const getOrganizedEventController = asyncHandler(async (req, res) => {
    ...
    const page = req.query.page || 1;
    const offset = req.query.offset || 10;
    const { event, meta } = await getOrganizedEventServices(req.user, userId, Number(page), Number(offset));
    return resHandler(res, 200, true, "event retrived successfully", event, meta);
});
export const postEventImagesController = asyncHandler(async (req, res) => {
    const { eventId } = req.params;
    if (!req.files)
        throw new apiError(400, "filepath missing");
    const files = req.files;
    const eventImage = await postEventImageServices(req.user, files, eventId);
    return resHandler(res, 201, true, "event image added successfully", eventImage);
});
//update event
export const updateEventController = asyncHandler(async (req, res) => {
    ...
    const data = req.body;
    const updateEvent = await updateEventServices(eventId, data, req.user);
    return resHandler(res, 200, true, "event updated successfully", updateEvent);
});
//delete event image
export const deleteEventImagesController = asyncHandler(async (req, res) => {
    const { eventImageId } = req.params;
    if (Array.isArray(eventImageId))
        throw new apiError(400, "params shouldn't be an array");
    const deleteEventImage = await deleteEventImagesServices(eventImageId, req.user);
